...
void HerkulexServo::individual_move(uint16_t pos_degres, uint16_t time_ms, HerkulexLed led = HerkulexLed::Ignore) {
  
  m_bus->prepareIndividualMove();
  uint16_t pos_servo = 512 + uint16_t(pos_degres / 0.325f);
  uint8_t time_servo = uint8_t(time_ms / 11.2f);
  setPosition(pos_servo,time_servo, led);
  m_bus->executeMove();
  m_bus->update();
  delay(time_ms);
